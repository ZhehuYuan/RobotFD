...
dist(q_{j+1},(p_i,p_{i+1})),
]

and

[
w^{\mathrm{right}}_{i,j}
========================

dist(p_{i+1},(q_j,q_{j+1})).
]

Here (w^{\mathrm{top}}*{i,j}) is the weight of leaving cell ((i,j)) through its top edge, and (w^{\mathrm{right}}*{i,j}) is the weight of leaving cell ((i,j)) through its right edge.
...
\max(dist(p_0,q_0),dist(p_{m-1},q_{n-1})).
]
...
For each valid cell ((i,j)), define:

[
T_{i,j}
]

as the minimum possible bottleneck value among all monotone VE paths that reach the top edge of cell ((i,j)), and

[
R_{i,j}
]

as the minimum possible bottleneck value among all monotone VE paths that reach the right edge of cell ((i,j)).
